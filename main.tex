...
\begin{itemize}
    ...
    \begin{itemize}
        \item Cấu hình: Thông thường sử dụng 3 bánh xe đa hướng bố trí lệch nhau $120^{\circ}$ \cite{HYHan2014}. Góc tiếp xúc của bánh xe với mặt phẳng ngang thường được đặt ở mức $45^{\circ}$ hoặc $50^{\circ}$ để tối ưu hóa việc truyền lực \cite{Kumagai2008, Lima2018}.
        \item Biến thể: Một số thiết kế tập trung vào chi phí thấp và đơn giản hóa bằng cách sử dụng động cơ bước truyền động trực tiếp không hộp số để giảm độ rơ (backlash) \cite{Kumagai2008}. Các thiết kế hiện đại hơn như robot MiaPURE tập trung vào khả năng chịu tải lớn (lên đến 60-80kg) để chở người hoặc hàng hóa, sử dụng bánh xe omni chịu lực cao và động cơ Quasi-Direct-Drive (QDD) \cite{Xiao2023}.
...
\begin{itemize}
    ...
    \item Mô hình phẳng: Để đơn giản hóa, chuyển động của Ballbot thường được tách thành hai mô hình con lắc ngược bánh xe (WIP) độc lập trên hai mặt phẳng trực giao. Giả định này hợp lý khi góc nghiêng nhỏ và sự ghép nối giữa các mặt phẳng là không đáng kể \cite{Xiao2023, Nagarajan2013, HYHan2014}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item LQR-PI: Xiao và các cộng sự \cite{Xiao2023} đề xuất bộ điều khiển tầng LQR-PI, trong đó vòng ngoài LQR tính toán mô-men xoắn tối ưu, còn vòng trong PI xử lý các ma sát phi tuyến và ma sát tĩnh (stiction) mà mô hình LQR thường bỏ qua. Phương pháp này giúp robot MiaPURE đạt vận tốc tới 2.3 m/s và phanh dừng trong 2 giây.
        \item Fuzzy PD: Cai et al. đề xuất bộ điều khiển Fuzzy PD tầng (Cascade Fuzzy PD) để xử lý các vùng phi tuyến ở góc nghiêng lớn, kết hợp với điều khiển vị trí và điều khiển góc xoay (yaw) \cite{Cai2019}.
        \item Observer \& Feedback: Park sử dụng bộ bù (compensator) bao gồm một bộ quan sát (observer) vòng kín và phản hồi trạng thái đầy đủ để giảm thiểu sai số mô hình \cite{Park2024}.
    \end{itemize}

    \item Ước lượng góc nghiêng: Việc sử dụng bộ lọc Kalman hoặc bộ lọc bù (Complementary Filter) để hợp nhất dữ liệu từ gia tốc kế và con quay hồi chuyển là tiêu chuẩn để có được góc nghiêng chính xác, loại bỏ nhiễu và trôi \cite{HYHan2014, Park2024}.
\end{itemize}
...
\subsubsection{Nhóm trọng tâm nội tại: Gồm robot dạng cầu (spherical robot) và robot đơn bánh vành lớn (monowheel)}
...
% 9
\bibitem{HYHan2014} H. Y. Han, T. Y. Han and H. S. Jo, "Development of omnidirectional self-balancing robot," 2014 IEEE International Symposium on Robotics and Manufacturing Automation (ROMA), Kuala Lumpur, Malaysia, 2014, pp. 57-62, doi: 10.1109/ROMA.2014.7295862.
% 10
