...
\begin{abstract}
Bài báo này trình bày một khảo sát toàn diện và hệ thống hóa về các cơ cấu di chuyển của robot di động, tập trung vào hai thành phần cốt lõi: hệ thống bánh xe và cấu trúc khung robot. Nghiên cứu phân loại chi tiết các loại bánh xe từ tiêu chuẩn, đa hướng (Mecanum/omni) đến các thiết kế đặc biệt như bánh xe hình cầu và bánh xe Origami biến hình. Đồng thời, bài báo phân tích sâu các cấu trúc robot phổ biến dựa trên số lượng bánh xe (từ robot một bánh, robot hai bánh, đến các robot đa bánh Rocker-Bogie và cấu trúc lai chân-bánh). Bên cạnh việc mô tả nguyên lý cơ khí, báo cáo còn thảo luận về các thách thức hiện đại trong mô hình hóa động học, xử lý ma sát phi tuyến và các chiến lược điều khiển tiên tiến. Kết quả khảo sát cung cấp cơ sở lý luận quan trọng cho việc lựa chọn thiết kế tối ưu trong các ứng dụng thực tế từ logistics đến thám hiểm địa hình.
\end{abstract}
...
Tuy nhiên, sự đa dạng về yêu cầu ứng dụng — từ di chuyển tốc độ cao trên đường nhựa đến khả năng xoay sở linh hoạt trong không gian chật hẹp — đã dẫn đến sự ra đời của hàng loạt thiết kế bánh xe và cấu trúc khung khác nhau. 
Việc lựa chọn giữa bánh xe tiêu chuẩn, bánh xe đa hướng, hay các cấu trúc dẫn động vi sai, cấu trúc lái Ackermann... không chỉ ảnh hưởng đến khả năng cơ động mà còn quyết định mô hình động học và độ phức tạp của hệ thống điều khiển \cite{campion1993, lynch2017}.
...
Đặc điểm chung của nhóm này là chúng tạo ra các ràng buộc phi đa hướng \cite{siegwart2011, rasam2016}, 
nghĩa là tại một thời điểm, bánh xe chỉ có thể lăn hiệu quả theo một hướng và không thể trượt theo phương của trục bánh xe trong điều kiện lý tưởng.
...
Nó chỉ có một bậc tự do là quay quanh trục chính của nó \cite{chung2008, mikova2013}.
...
buộc robot phải thực hiện các quỹ đạo phức tạp (như đỗ xe song song) để đến được vị trí mong muốn thay vì di chuyển thẳng. Tuy nhiên, chính ràng buộc này lại mang lại sự ổn định hướng vượt trội khi di chuyển ở tốc độ cao, giúp robot giữ lộ trình tốt hơn so với các loại bánh đa hướng.

Các phương trình (\ref{eq1}) và (\ref{eq2}) dựa trên giả thuyết lý tưởng về lăn không trượt và tiếp xúc điểm cứng tuyệt đối. 
Tuy nhiên, trong các ứng dụng thực tế, đặc biệt là với các robot vận tải (AGV) mang tải trọng lớn hoặc robot nông nghiệp di chuyển trên địa hình mềm, giả thuyết này thường bị vi phạm:
...
\begin{enumerate}
    ...
    \item Vì mô hình hóa không thể loại bỏ hoàn toàn sai số tích lũy, dữ liệu từ bánh xe bắt buộc phải được kết hợp với các cảm biến ngoại vi (như IMU, LiDAR) thông qua các bộ lọc ước lượng trạng thái (như EKF) để đảm bảo độ chính xác vị trí trong thời gian dài \cite{thrun2010, moore2014}.
\end{enumerate}
...
\begin{enumerate}
    \item Ma sát xoay và hiện tượng cày lốp: Một vấn đề vật lý nghiêm trọng của bánh xe lái là hiện tượng ma sát xoay khi robot cần đổi hướng mà vận tốc lăn thấp hoặc bằng không. Để thay đổi góc $\beta$, động cơ lái phải thắng được mô-men ma sát trượt xoay rất lớn giữa bề mặt lốp và mặt đường \cite{gillespie1992}. Điều này không chỉ gây mài mòn lốp nhanh chóng mà còn đòi hỏi động cơ lái phải có mô-men xoắn cực đại lớn, làm tăng kích thước và tiêu thụ năng lượng của hệ thống.
    \item Giới hạn tốc độ lái và Phối hợp điều khiển: Trong lý thuyết, góc lái $\beta$ có thể thay đổi tức thời. Tuy nhiên, trong thực tế, tốc độ thay đổi góc lái ($\beta$) bị giới hạn bởi động lực học của cơ cấu chấp hành. Nếu bộ điều khiển yêu cầu thay đổi hướng quá nhanh so với vận tốc lăn của robot, bánh xe sẽ bị trượt ngang, vi phạm ràng buộc (4). Do đó, các thuật toán điều khiển hiện đại cho robot dùng bánh lái (như Ackermann hoặc Tricycle) đòi hỏi sự phối hợp đồng bộ chặt chẽ giữa vận tốc lăn $\varphi$ và vận tốc lái $\beta$ để đảm bảo quỹ đạo mượt mà \cite{deluca1998}.
    \item Sai số cơ khí: Hệ thống truyền động lái (thường dùng bánh răng hoặc trục vít) luôn tồn tại độ rơ. Sai số này làm cho góc lái thực tế lệch so với góc lái đo được từ encoder, dẫn đến sai số trong bài toán định vị lớn hơn so với bánh xe cố định.
\end{enumerate}
...
Sự khác biệt cốt lõi giữa \textit{bánh xe xoay tự do} và bánh lái tiêu chuẩn nằm ở tham số $d$ (khoảng cách lệch tâm giữa trục lái và điểm tiếp xúc) trong phương trình (6). Sự xuất hiện của thành phần $d\dot{\beta}$ mang ý nghĩa vật lý quan trọng: mọi chuyển động ngang (trượt) của khung robot tại điểm gắn kết đều có thể được triệt tiêu bằng một tốc độ xoay $\dot{\beta}$ phù hợp của bánh xe.
Điều này dẫn đến một kết luận quan trọng: Nếu khớp xoay là tự do (ma sát thấp), \textit{bánh xe xoay tự do} không áp đặt bất kỳ ràng buộc động học nào lên chuyển động tổng thể của robot [17]. Robot có thể di chuyển theo bất kỳ quỹ đạo nào, và \textit{bánh xe xoay tự do} sẽ tự động xoay để đi theo chuyển động đó.

Các vấn đề hiện đại và thách thức động lực học: Mặc dù về mặt hình học, bánh xe xoay tự do rất hoàn hảo để làm bánh phụ trợ, nhưng về mặt động lực học, chúng gây ra nhiều vấn đề phức tạp cho các robot hiện đại:

\begin{enumerate}
    \item Hiện tượng rung lắc: Đây là một vấn đề kinh điển nhưng vẫn là thách thức trong thiết kế robot tốc độ cao. Khi di chuyển nhanh, \textit{bánh xe xoay tự do} thường xảy ra hiện tượng dao động tự kích quanh trục lái, gây rung lắc dữ dội cho thân robot. Điều này xuất phát từ sự tương tác phức tạp giữa độ đàn hồi của lốp, độ lệch tâm $d$ và ma sát, điều mà các mô hình động học (5) và (6) không mô tả được [18]. Các robot hiện đại thường phải sử dụng bộ giảm chấn hoặc thiết kế tối ưu hóa độ cứng lốp để triệt tiêu hiện tượng này.
    \item Hiệu ứng đảo chiều: Khi robot đổi hướng di chuyển đột ngột (ví dụ: từ tiến sang lùi), \textit{bánh xe xoay tự do} phải thực hiện một hành trình xoay 180 độ quanh trục đứng để căn chỉnh lại. Quá trình này tạo ra một khoảng "trễ" và một lực phản hồi bất định lên khung robot, gây ra sai số vị trí lớn (Odometry error) và làm quỹ đạo thực tế bị giật cục. Trong các thuật toán định vị hiện đại (SLAM), hành vi khó đoán này của \textit{bánh xe xoay tự do} thường được coi là nhiễu (noise).
    \item Xu hướng mới: (\textit{bánh xe xoay tự do chủ động}): Để khắc phục tính bị động, một xu hướng nghiên cứu hiện đại là Active Caster (hay Powered Caster). Thay vì để bánh tự xoay do ma sát, người ta gắn động cơ vào cả trục lăn và trục lái. Khi đó, hệ thống trở thành một mô đun truyền động đa hướng mạnh mẽ, cho phép robot di chuyển holonomic mà không cần dùng bánh Mecanum đắt tiền [19].
\end{enumerate}

Tóm lại, \textit{bánh xe xoay tự do} là giải pháp rẻ tiền và hiệu quả để duy trì cân bằng tĩnh, nhưng sự phức tạp về động lực học của nó đòi hỏi sự cân nhắc kỹ lưỡng trong thiết kế robot chính xác cao.
...
Cấu tạo và nguyên lý: Bánh xe omni (hay còn gọi là Universal Wheel) bao gồm một vành bánh xe chính và các con lăn nhỏ được gắn dọc theo chu vi. Đặc điểm hình học cốt lõi là trục quay của các con lăn này vuông góc ($90 ^{\circ}$) với trục quay chính của bánh xe \cite{patil2024, kanjanawanishkul2015}. Nguyên lý hoạt động dựa trên sự tách biệt chuyển động:

\begin{itemize}
    \item Động cơ dẫn động trục chính tạo ra lực đẩy chủ động theo hướng dọc.
    \item Các con lăn tự do cho phép bánh xe trượt thụ động theo phương ngang. Sự kết hợp của nhiều bánh xe omni (thường là 3 hoặc 4) cho phép tổng hợp véc-tơ vận tốc theo bất kỳ hướng nào \cite{komori2016, kanjanawanishkul2015}.
\end{itemize}

Các biến thể cấu trúc và giảm chấn: Một nhược điểm cố hữu của thiết kế omni cổ điển là sự không liên tục của điểm tiếp xúc. Giữa các con lăn luôn tồn tại khoảng hở, gây ra rung động dọc khi bánh xe quay. Ferrière và các cộng sự \cite{ferriere1996} đã phân tích rằng biên độ rung động này phụ thuộc trực tiếp vào kích thước khe hở và bán kính con lăn. Để khắc phục, các biến thể cấu trúc hiện đại đã được phát triển:

\begin{itemize}
    \item Bánh omni kép: Sử dụng hai đĩa bánh xe ghép lại, lệch pha nhau để đảm bảo biên dạng tiếp xúc liên tục. Tuy nhiên, thiết kế này làm tăng bề rộng và khối lượng bánh xe.
    \item Thiết kế con lăn Xen kẽ: Các con lăn được thiết kế đan xen để giảm thiểu khe hở mà không cần dùng bánh kép, giúp giảm rung động hiệu quả.
\end{itemize}

\textbf{Phân tích Động lực học Trượt}: Trong các nghiên cứu hiện đại, mô hình động học lý tưởng (lăn không trượt) đã được chứng minh là không đủ chính xác. Williams và các cộng sự \cite{williams2002} đã phát hiện ra rằng phần vật liệu cứng nằm giữa các con lăn đóng vai trò quan trọng trong việc gây ra hiện tượng trượt, ngay cả ở tốc độ thấp. Nghiên cứu thực nghiệm chỉ ra rằng hệ số ma sát trong hướng lăn ($\mu_W$) và hướng trượt ngang ($\mu_T$) là khác nhau đáng kể (ví dụ: $\mu_W \approx 0.26$ so với $\mu_T \approx 0.09$ trên bề mặt giấy). Do đó, các bộ điều khiển robot hiện đại cần tích hợp mô hình ma sát phi tuyến để bù trừ sai số trượt này, thay vì chỉ dựa vào mô hình hình học thuần túy.

Đột phá về Vật liệu: Xu hướng "nhẹ hóa" robot đang thúc đẩy việc thay thế các vật liệu kim loại truyền thống. Nghiên cứu mới nhất của Patil và các cộng sự \cite{patil2024} đã đề xuất sử dụng vật liệu Composite sợi Carbon gia cường (Carbon Fiber Reinforced Polymer) để chế tạo đĩa bánh xe omni thay cho nhôm.

\begin{itemize}
    \item Sử dụng phương pháp đắp tay với nhựa epoxy, đĩa bánh xe sợi carbon đạt được độ bền uốn lên tới 2.2 kN.
...
\begin{itemize}
    \item Thiết kế này sử dụng cơ cấu vi sai để dẫn động chủ động cả trục bánh xe chính và trục của các con lăn con.
    \item Kết quả là một bánh xe duy nhất có thể chủ động di chuyển theo hướng bất kỳ mà không cần phụ thuộc vào lực kéo của các bánh xe khác, mở ra khả năng thiết kế các loại xe vận chuyển đa hướng với cấu hình linh hoạt hơn.
\end{itemize}
...
\textbf{Lịch sử và Cấu hình Hình học:} Được phát minh vào năm 1973 bởi Bengt Ilon tại Thụy Điển \cite{ilon1975, wikipedia_mecanum_wheel}, bánh xe Mecanum có cấu tạo gồm một trục chính và các con lăn thụ động bao quanh. Đặc điểm nhận dạng là trục của các con lăn tạo một góc $\alpha = 45^{\circ}$ so với trục bánh xe.
...
\begin{itemize}
    ...
    \item \textbf{Bố trí:} Để robot hoạt động đúng, 4 bánh xe phải được lắp đặt theo quy tắc hình chữ nhật hoặc hình vuông, sao cho các trục con lăn tạo thành hình chữ X hướng vào tâm robot khi nhìn từ trên xuống \cite{salih2006, hasana2021}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Vấn đề Trượt:} Mặc dù mô hình động học lý thuyết (dựa trên ma trận Jacobian) rất hoàn hảo \cite{dyakov2024}, Ignatiev và các cộng sự \cite{ignatiev2016} và Salih và các cộng sự \cite{salih2006} đều nhấn mạnh rằng trong thực tế, hiện tượng trượt là không thể tránh khỏi do diện tích tiếp xúc nhỏ (điểm) của con lăn. Điều này làm giảm độ chính xác của phương pháp định vị bằng việc đếm xung encoder, đòi hỏi các giải pháp bù trừ sai số.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Phân tích Rung động:} Bae và Kang \cite{bae2016} đã đo đạc thực nghiệm và phát hiện ra gia tốc rung động dọc trục có tính bất đối xứng, tăng mạnh ở tốc độ cao.
    \item \textbf{Tối ưu hóa Topo:} Để khắc phục, Doroliat và các cộng sự \cite{doroliat2022} đề xuất sử dụng thuật toán tối ưu hóa topo để thiết kế lại cấu trúc bên trong của lớp cao su bọc con lăn. Bằng cách thay đổi độ dày vật liệu tại các vị trí khác nhau, họ tạo ra độ cứng tiếp xúc đồng nhất, giúp giảm rung động tới 30\%.
\end{itemize}

\textbf{Hệ thống Điều khiển và Tích hợp:} Sự phát triển của bánh Mecanum gắn liền với sự tiến bộ của hệ thống nhúng.
...
\begin{itemize}
    ...
    \item Bánh cầu chủ động: Đây là một cơ cấu cơ khí cực kỳ phức tạp, trong đó quả cầu được chủ động điều khiển để tạo lực đẩy.
          Quả cầu được giữ trong một hốc và tiếp xúc trực tiếp với các con lăn hoặc động cơ được đặt bên trong cơ cấu. Bằng cách điều khiển các con lăn này 
          ...
          Một bánh xe cầu chủ động là một cơ cấu truyền động đa hướng hoàn chỉnh. 
...
\begin{itemize}
    ...
    \item Ước lượng góc nghiêng: Việc sử dụng bộ lọc Kalman hoặc bộ lọc bù để hợp nhất dữ liệu từ gia tốc kế và con quay hồi chuyển là tiêu chuẩn để có được góc nghiêng chính xác, loại bỏ nhiễu và trôi \cite{HYHYHan2014, Park2024}.
\end{itemize}
...
Nguyên lý di chuyển của robot dạng cầu chủ yếu dựa vào việc thay đổi trọng tâm hoặc bảo toàn mô-men động lượng. Các thiết kế có thể được phân loại thành ba nhóm chính:
...
\begin{itemize}
    \item Cấu trúc Điều khiển Tầng (Cascaded Control): Kamga và Rachid \cite{kamga1996} đã đặt nền móng với cấu trúc điều khiển tách biệt cho vận tốc và góc lái. Phát triển trên nền tảng đó, Precup và các cộng sự \cite{Precup2006} đề xuất kiến trúc điều khiển mờ kết hợp với trường thế năng nhân tạo, giúp robot xử lý các hành vi phi tuyến của cấu trúc ba bánh khi tránh vật cản.
    \item Cấu trúc Thích nghi và Tối ưu: Để đối phó với sự thay đổi trọng tâm và ma sát, Pătraşcu và Gheorghe \cite{patrascu2021} sử dụng kiến trúc điều khiển P-D được tinh chỉnh trực tuyến bằng giải thuật di truyền. Đồng thời, Ketemaw và Seid \cite{Ketemaw2022} đề xuất kiến trúc điều khiển trượt tích hợp logic mờ để giám sát chỉ số RI, tự động kích hoạt phanh vi sai cầu sau để chống lật chủ động.
\end{itemize}
...
\begin{itemize}
    \item Trên các robot dẫn động trượt thừa dẫn động, các bánh xe thường sinh ra nội lực chống lại nhau. Các nghiên cứu của Liao và các cộng sự [40], [41] tập trung vào các thuật toán phân phối mô-men xoắn tối ưu. Mục tiêu là tìm ra tổ hợp lực kéo của 4 bánh sao cho robot vẫn bám quỹ đạo mong muốn nhưng tiêu tốn năng lượng ít nhất và giảm thiểu mài mòn lốp.
