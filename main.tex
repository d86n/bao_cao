...
\begin{enumerate}
    \item Các mô hình động học được bổ sung thêm các tham số về độ trượt (slip ratio) và độ cứng lốp để phản ánh chính xác hơn tương tác vật lý [].
    \item Vì mô hình hóa không thể loại bỏ hoàn toàn sai số tích lũy, dữ liệu từ bánh xe (Odometry) bắt buộc phải được kết hợp với các cảm biến ngoại vi (như IMU, LiDAR) thông qua các bộ lọc ước lượng trạng thái (như EKF) để đảm bảo độ chính xác vị trí trong thời gian dài [].
\end{enumerate}
