...
Để hệ thống hóa các cấu trúc robot đa dạng hiện nay, báo cáo sử dụng phương pháp phân loại cấu trúc kinh điển dựa trên đặc tính động học được đề xuất bởi W. Chung và K. Iagnemma \cite{chung2008}. Theo đó, mỗi cấu hình robot (WMR) được đặc trưng bởi một cặp chỉ số (m,s), trong đó:

\begin{itemize}
    \item $m$ (mobility degree): Bậc linh hoạt, thể hiện số lượng bậc tự do vận tốc tức thời mà robot có thể thực hiện trong mặt phẳng (không tính đến việc đổi hướng bánh xe).
    \item $s$ (steerability degree): Bậc lái, thể hiện số lượng biến độc lập có thể điều khiển hướng của các bánh xe.
\end{itemize}

Dựa trên cặp chỉ số này, robot di động được chia thành 5 lớp cấu trúc chính:

\begin{enumerate}
    \item Loại (3,0) - Robot Toàn hướng (Omnidirectional Robots)
    \begin{itemize}
        \item \textbf{Đặc điểm:} $m = 3$, $s = 0$.
        \item \textbf{Mô tả:} Robot không sử dụng bánh lái mà trang bị hoàn toàn các bánh xe đặc biệt (như bánh Mecanum, bánh omni, hoặc bánh caster).
        \item \textbf{Động học:} Robot có khả năng di chuyển tự do theo cả 3 bậc tự do trong mặt phẳng ($v_x$, $v_y$, $\omega$) ngay lập tức mà không cần phải thực hiện thao tác xoay thân xe để đổi hướng.
    \end{itemize}

    \item Loại (2,0) - Robot Dẫn động Vi sai (Differential Drive Robots)
    \begin{itemize}
        \item \textbf{Đặc điểm:} $m = 2$, $s = 0$.
        \item \textbf{Mô tả:} Robot không có bánh lái. Cấu trúc bao gồm một hoặc nhiều bánh xe cố định được gắn trên cùng một trục chung.
        \item \textbf{Động học:} Trục chung tạo ra một ràng buộc không trượt ngang (non-holonomic constraint), giới hạn khả năng di chuyển tức thời của robot trong không gian 2 chiều (chỉ có thể tiến/lùi và quay, không thể đi ngang). Đây là lớp robot phổ biến nhất trong các ứng dụng cơ bản.
    \end{itemize}

    \item Loại (2,1) - Robot Lái Đồng bộ không Bánh cố định
    \begin{itemize}
        \item \textbf{Đặc điểm:} $m = 2$, $s = 1$.
        \item \textbf{Mô tả:} Robot không có bánh xe cố định và có ít nhất một bánh lái. Nếu có nhiều bánh lái, hướng của chúng phải được phối hợp đồng bộ (ví dụ: Cấu trúc Synchro-drive).
        \item \textbf{Động học:} Mặc dù có bánh lái, nhưng do sự đồng bộ hóa, bậc linh hoạt của robot vẫn bị giới hạn ở mức 2 trong mặt phẳng chuyển động.
    \end{itemize}

    \item Loại (1,1) - Robot Lái có Trục cố định (Tricycle/Car-like)
    \begin{itemize}
        \item \textbf{Đặc điểm:} $m = 1$, $s = 1$.
        \item \textbf{Mô tả:} Cấu trúc kết hợp giữa một hoặc nhiều bánh cố định trên một trục chung và một hoặc nhiều bánh lái.
        \item \textbf{Điều kiện:} Tâm quay của bánh lái không được nằm trên trục của bánh cố định.
        \item \textbf{Động học:} Khả năng di chuyển tức thời bị giới hạn nghiêm ngặt xuống 1 chiều (di chuyển dọc theo hướng bánh xe). Hướng di chuyển này được xác định bởi góc quay của bánh lái ($s = 1$). Các cấu trúc tiêu biểu: Steered Tricycle (xe ba bánh), Ackermann (xe ô tô).
    \end{itemize}

    \item Loại (1,2) - Robot Đa trục lái (Multi-steering Robots)
    \begin{itemize}
        \item \textbf{Đặc điểm:} $m = 1$, $s = 2$.
        \item \textbf{Mô tả:} Robot không có bánh xe cố định nhưng có ít nhất hai bánh lái. Nếu có nhiều hơn hai bánh lái, chúng phải được phối hợp thành hai nhóm điều khiển độc lập.
        \item \textbf{Động học:} Tương tự loại (1, 1), chuyển động tức thời bị giới hạn ở 1 chiều, nhưng hướng di chuyển được xác định bởi sự phối hợp góc của hai nhóm bánh lái ($s = 2$). Ví dụ: Các robot 4 bánh lái độc lập (4WS) ở chế độ lái phức tạp.
    \end{itemize}
\end{enumerate}
...
Điều khiển Dự báo Mô hình (MPC - Model Predictive Control) Đây là xu hướng nghiên cứu nóng nhất hiện nay. Jespersen và các cộng sự (2020) [73] đã áp dụng thành công Path-Following MPC.
...
\bibitem{qiu2018} Q. Qiu và các cộng sự, “Extended Ackerman Steering Principle for the coordinated movement control of a four wheel drive agricultural mobile robot,” Computers and Electronics in Agriculture, vol. 152, pp. 40-50, Jul. 2018, doi: 10.1016/j.compag.2018.06.036.
...
\bibitem{jespersen2020} T. K. Jespersen và các cộng sự, "Path-Following Model Predictive Control of Ballbots," 2020 IEEE International Conference on Robotics and Automation (ICRA), Paris, France, 2020, pp. 1498-1504, doi: 10.1109/ICRA40945.2020.9196634.
