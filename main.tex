...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.35\textwidth]{figures/fig_ilon_1975.pdf}
    \caption{Sáng chế bánh xe Mecanum của Ilon \cite{ilon1975}}
    \label{fig:fig_ilon_1975}
\end{figure}
...
\begin{itemize}
    \item \textbf{Điều khiển cơ bản:} Salih và các cộng sự \cite{salih2006} trình bày kiến trúc điều khiển kinh điển sử dụng vi điều khiển (như BasicStamp) kết hợp với mạch cầu H (H-bridge drivers) để điều xung PWM độc lập cho 4 động cơ DC, cho phép thực hiện các chuyển động cơ bản.
    \item \textbf{Điều khiển Tự hành Nâng cao:} Ignatiev và các cộng sự \cite{ignatiev2016} đã nâng cấp hệ thống lên mức tự hành (autonomous) bằng cách tích hợp Thị giác máy tính (Computer Vision). Robot được trang bị camera và máy tính nhúng để nhận diện mẫu (pattern recognition) và bám theo đối tượng (tracking), khắc phục nhược điểm sai số vị trí của bánh Mecanum.
    \item \textbf{Mô phỏng Động lực học:} Để kiểm chứng thuật toán trước khi chế tạo, Dosoftei và các cộng sự \cite{dosoftei2020} và Zimmermann và các cộng sự \cite{zimmermann2014} đề xuất các phương pháp mô phỏng động lực học đa vật thể (multibody dynamics) và mô hình hóa giản lược, giúp tính toán chính xác lực va chạm và mô-men xoắn yêu cầu.
\end{itemize}
