...
Đây là loại bánh xe toàn hướng đầu tiên và phổ biến nhất, được cấp bằng sáng chế lần đầu bởi Grabowiecki vào năm 1919. Cấu tạo cơ bản gồm một vành bánh xe chính (Hub) và một loạt các con lăn nhỏ (Rollers) hình tang trống được gắn dọc theo chu vi.

\begin{itemize}
    \item Đặc tính hình học: Trục quay của các con lăn con luôn vuông góc (90 độ) với trục quay chính của bánh xe.
    \item Nguyên lý hoạt động:
    \begin{itemize}
        \item Khi động cơ quay trục chính: Bánh xe tạo ra lực đẩy dọc (Longitudinal force) như bánh xe thường.
        \item Khi có ngoại lực ngang: Các con lăn tự do quay, cho phép bánh xe trượt ngang (Lateral motion) với ma sát tối thiểu.
        \item Sự kết hợp này giúp robot di chuyển theo bất kỳ hướng nào mà không bị ràng buộc non-holonomic [43].
    \end{itemize}
\end{itemize}

Các Biến thể Cấu trúc Hiện đại (Modern Structural Variations): Nhược điểm chí mạng của bánh Omni cổ điển là biên dạng không tròn đều. Giữa các con lăn luôn có khoảng hở (gap), gây ra rung động mạnh khi di chuyển. Các nghiên cứu hiện đại đã phát triển các biến thể để khắc phục:

\begin{itemize}
    \item Bánh Omni Kép (Dual-row / Double Omni): Gồm hai đĩa bánh Omni đơn ghép lại, lệch pha nhau một nửa bước con lăn.
    \begin{itemize}
        \item Ưu điểm: Đảm bảo luôn có ít nhất một con lăn tiếp xúc với mặt đất tại mọi thời điểm, giúp triệt tiêu gần như hoàn toàn rung động và tiếng ồn. Đây là chuẩn mực cho các robot thi đấu Robocon/RoboCup hiện nay [43].
    \end{itemize}
    
    \item Bánh Omni Liên tục (Continuous-rim Omni): Sử dụng các con lăn có hình dáng đặc biệt đan xen nhau để tạo thành biên dạng tròn liên tục hoàn hảo. Loại này thường thấy trong các xe nâng hàng đa hướng (Forklifts) tải trọng lớn.
\end{itemize}

Phân tích Động lực học và Rung động (Dynamics \& Vibrations): Mặc dù lý thuyết động học giả định bánh xe là tròn tuyệt đối, thực tế bán kính hiệu dụng $r_{eff}$ của bánh Omni biến thiên tuần hoàn theo góc quay $\theta$.

\begin{equation}
    r_{eff}(\theta) = R_{hub} + r_{roller} \cos(\mod(\theta, \alpha))
\end{equation}

Sự biến thiên này tạo ra lực kích động tuần hoàn theo phương thẳng đứng, gây rung lắc thân robot và làm giảm độ chính xác của cảm biến Odometry. Các bộ điều khiển hiện đại phải sử dụng bộ lọc Kalman hoặc mô hình động lực học lốp để bù trừ sai số chu kỳ này [Ref A].

Ứng dụng và Xu hướng: Bánh Omni đang dần chuyển dịch từ robot giáo dục sang robot dịch vụ trong nhà (như robot hút bụi cao cấp, robot phục vụ bàn) nhờ ưu điểm cơ khí đơn giản và bền bỉ hơn so với bánh Mecanum (ít chi tiết chuyển động hơn, chịu bụi tốt hơn).
...
So sánh với Omni: Điểm khác biệt cốt lõi là góc nghiêng con lăn (45 độ so với 90 độ). Điều này khiến lực đẩy của bánh Mecanum luôn có một thành phần lực dọc trục (axial force) rất lớn.

Hệ quả: Để đi thẳng, các lực dọc trục của 4 bánh phải triệt tiêu nhau hoàn toàn. Điều này gây ra ứng suất lớn lên trục động cơ và tiêu tốn năng lượng vô ích (do ma sát nội tại). Do đó, bánh Mecanum thường có hiệu suất năng lượng thấp hơn bánh Omni đơn [44].
...
\subsection{Cấu trúc ba bánh}
Cấu trúc ba bánh là một trong những thiết kế kinh điển và phổ biến nhất trong robot công nghiệp và xe tự hành (AGV). Khác với cấu trúc vi sai (điều hướng bằng chênh lệch vận tốc), cấu trúc này tách biệt hoàn toàn chức năng tạo lực đẩy và chức năng điều hướng [1]. Về mặt động học, đây là một hệ thống phi đa hướng (non-holonomic) với bán kính quay vòng bị giới hạn bởi góc lái cực đại [2].
...
Cấu trúc đa hướng cho phép robot di chuyển theo bất kỳ hướng nào trên mặt phẳng (3 bậc tự do $v_x$, $v_y$, $\omega$) mà không cần thay đổi hướng của thân xe. Dựa trên cơ chế tạo lực ngang, ta có thể phân loại thành hai nhóm cấu trúc chính:

\begin{enumerate}
    \item Nền tảng sử dụng Bánh xe Đa hướng
    Đây là nhóm phổ biến nhất trong nghiên cứu và robot thi đấu (RoboCup) nhờ điều khiển đơn giản (không có điểm kỳ dị động học).
    \begin{itemize}
        \item Cấu trúc 3 bánh Omni ($120^o$):
        \begin{itemize}
            \item Mô tả: Ba bánh Omni bố trí đối xứng lệch nhau 120 độ.
            \item Đặc điểm: Là cấu trúc holonomic tối thiểu (dùng ít động cơ nhất). Tuy nhiên, độ ổn định kém (dễ bị lật do hình tam giác) và tải trọng không đều [Ref].
        \end{itemize}

        \item Cấu trúc 4 bánh Omni (90 độ):
        \begin{itemize}
            \item Mô tả: Bốn bánh Omni đặt vuông góc (hình chữ thập hoặc chữ X).
            \item Đặc điểm: Tăng độ ổn định và khả năng chịu tải so với 3 bánh. Tuy nhiên, bánh xe Omni chịu mài mòn nhanh và rung động nhiều khi di chuyển.
        \end{itemize}

        \item Cấu trúc 4 bánh Mecanum (Rectangular):
        \begin{itemize}
            \item Mô tả: Bốn bánh Mecanum đặt song song ở 4 góc (như xe ô tô).
            \item Đặc điểm: Phổ biến nhất trong công nghiệp (xe nâng đa hướng). Ưu điểm là hình dáng chữ nhật dễ bố trí linh kiện và không gian chứa hàng lớn. Nhược điểm là lực ma sát trượt dọc trục tiêu tốn năng lượng lớn [Ref].
        \end{itemize}
    \end{itemize}

    \item Nền tảng Bánh lái Độc lập (Swerve Drive / Active Caster)
    Đây là xu hướng "hiện đại" đang thay thế dần Mecanum trong các ứng dụng công nghiệp hạng nặng (như robot KUKA KMP hay các AGV thế hệ mới).

    \begin{itemize}
        \item Mô tả: Sử dụng 4 (hoặc nhiều hơn) mô-đun bánh xe tiêu chuẩn. Mỗi mô-đun có 2 động cơ: một để lăn bánh và một để xoay hướng lái (steering) 360 độ vô hạn (infinite rotation).
        \item Cơ chế hoạt động: Bằng cách phối hợp góc lái và vận tốc của tất cả các bánh, robot có thể đi ngang, đi chéo hoặc xoay tại tâm bất kỳ (Swerve maneuver). Đây được gọi là hệ thống Pseudo-holonomic (Giả toàn hướng) hoặc Omni-steer.
        \item Ưu điểm vượt trội (Vấn đề hiện đại):
        \begin{itemize}
            \item Sử dụng lốp cao su tiêu chuẩn → Khả năng chịu tải và độ bám (traction) cao gấp nhiều lần bánh Mecanum/Omni con lăn nhựa.
            \item Không bị rung động, vận hành êm ái trên nền nhà xưởng không phẳng.
        \end{itemize}

        \item Thách thức: Cấu trúc cơ khí và thuật toán điều khiển phức tạp hơn rất nhiều (cần xử lý các điểm kỳ dị khi bánh xe phải xoay 180 độ để đảo chiều) [Ref].
    \end{itemize}
\end{enumerate}
...
Cấu trúc dẫn động bánh cầu (ballbot) đại diện cho một bước chuyển dịch mô hình trong thiết kế robot di động: từ sự ổn định tĩnh của các robot đa bánh truyền thống (như vi sai, Ackerman) sang trạng thái cân bằng động trên một điểm tiếp xúc duy nhất. Đây là ví dụ điển hình nhất cho lớp hệ thống cơ điện tử thiếu dẫn động và không ổn định tự nhiên, đặt ra những thách thức lớn nhất cho lý thuyết điều khiển hiện đại \cite{lauwers2006, hollis2006}.

\subsubsection{Kiến trúc cơ khí và nguyên lý truyền động}
Nguyên lý "Chuột bi ngược" (Inverse Mouse-ball Drive) Khác với bánh xe thông thường, quả cầu của Ballbot cần quay tự do theo 3 bậc tự do (Roll, Pitch, Yaw), do đó không thể gắn trục động cơ trực tiếp vào tâm cầu. Giải pháp tiêu chuẩn hiện nay là sử dụng cơ chế truyền động ma sát gián tiếp \cite{lauwers2006}. Thân robot được đỡ trên quả cầu bởi một hệ thống các bánh xe đa hướng (Omni-wheels) gắn cố định vào khung. Lực truyền động được tạo ra bởi ma sát tiếp tuyến giữa con lăn của bánh Omni và bề mặt quả cầu.

Các cấu hình tối ưu hóa: Việc bố trí các bánh xe Omni ảnh hưởng trực tiếp đến khả năng điều khiển và sự ổn định của hệ thống. Có hai cấu hình chính được nghiên cứu:

\begin{itemize}
    \item Cấu hình 4 bánh (90 độ): Bốn bánh Omni đặt vuông góc. Ưu điểm là mô hình động học có thể tách biệt (decoupled) thành hai mặt phẳng chuyển động XZ và YZ độc lập, giúp đơn giản hóa bộ điều khiển. Tuy nhiên, nhược điểm lớn là siêu tĩnh (over-constrained); nếu quả cầu không hoàn hảo hoặc khung bị lệch, một bánh xe có thể bị hổng chân (mất tiếp xúc), gây mất điều khiển \cite{kumagai2008}.
    
    \item Cấu hình 3 bánh (120 độ): Ba bánh Omni đặt lệch nhau 120 độ và nghiêng một góc $\alpha$ so với phương ngang (thường là 45 độ). Đây là cấu hình tối ưu nhất hiện nay vì 3 điểm luôn tạo thành một mặt phẳng, đảm bảo tiếp xúc động học liên tục ngay cả khi quả cầu bị mòn hoặc méo nhẹ \cite{han2014,skrabel2013}.
\end{itemize}

Thách thức về Ma sát học: Hiệu suất của Ballbot phụ thuộc vào hệ số ma sát $\mu$ tại điểm tiếp xúc. Kumagai \cite{kumagai2008} đã phân tích kỹ lưỡng sự đánh đổi này:
\begin{itemize}
    \item Nếu $\mu$ thấp: Xảy ra hiện tượng trượt (slippage), khiến robot quay bánh nhưng không tạo ra góc nghiêng, dẫn đến ngã.
    \item Nếu $\mu$ quá cao: Gây tổn hao năng lượng lớn do ma sát cản lăn (rolling resistance) và làm mài mòn nhanh bề mặt quả cầu.
    \item Giải pháp vật liệu: Các thiết kế hiện đại thường sử dụng quả cầu cốt nhôm hoặc thép rỗng, phủ lớp Urethane hoặc cao su NBR có độ cứng Shore A phù hợp để tối ưu hóa độ bám \cite{jesus2024}.
\end{itemize}

\subsubsection{Mô hình hóa Động lực học}

Để điều khiển Ballbot, không thể sử dụng các mô hình động học hình học (Kinematics) đơn thuần, mà bắt buộc phải sử dụng mô hình động lực học vật rắn phức tạp. Hệ thống được mô hình hóa như một Con lắc ngược 3D trên một quả cầu lăn.

Phương trình Lagrange Các nghiên cứu gần đây như của Bonci \cite{bonci2016} và Abdelrahim \cite{abdelrahim2025} (2025) đã xây dựng các bộ phương trình động lực học chi tiết sử dụng phương pháp Lagrange:

\begin{equation}
    \dfrac{d}{dt} \left( \dfrac{\partial \mathcal{L}}{\partial \dot{q}} \right)
    - \dfrac{\partial \mathcal{L}}{\partial q} = \tau
\end{equation}

Trong đó:
\begin{itemize}
    \item $q \in \mathbb{R} ^n$: Vectơ các biến trạng thái (góc nghiêng thân $\theta$, vị trí bóng $x$, $y$ và góc xoay $\psi$).
    \item $\mathcal{L} = \mathcal{K} - \mathcal{P}$: Hàm Lagrangian (Hiệu số giữa Động năng và Thế năng).
    \item $\tau$: Vector mô-men xoắn tổng quát từ các động cơ.
\end{itemize}

Phương trình này dẫn đến mô hình động lực học dạng ma trận:

\begin{equation}
    M(q) \ddot{q} + C(q, \dot{q}) \dot{q} + G(q) + F(\dot{q}) = B \tau
\end{equation}

Trong đó $M(q)$ là ma trận quán tính, $C(q, \dot{q})$ là ma trận Coriolis và lực ly tâm, $G(q)$ là vector trọng lực (thành phần gây mất ổn định), và $F(\dot{q})$ là ma sát nhớt.

Đặc tính "Pha không tối thiểu" (Non-minimum Phase) Một đặc điểm động lực học quan trọng được Nagarajan [61], [74] phân tích sâu là hành vi pha không tối thiểu. Do tính chất thiếu dẫn động, robot không thể trực tiếp tạo ra gia tốc tịnh tiến.

\begin{itemize}
    \item Quy trình di chuyển: Để đi từ A đến B, robot buộc phải thực hiện một quy trình nghịch lý: đầu tiên phải tăng tốc về phía ngược lại (-B) để mượn lực quán tính làm thân robot ngã về phía B, tạo ra mô-men trọng trường giúp tăng tốc.
    \item Hệ quả: Điều này làm phức tạp hóa bài toán lập kế hoạch quỹ đạo (Trajectory Planning), đòi hỏi các thuật toán phải có khả năng "nhìn trước" (look-ahead) để tính toán thời điểm nghiêng người hợp lý.
\end{itemize}

\subsubsection{Chiến lược Điều khiển Hiện đại}

Giữ thăng bằng cho Ballbot là bài toán điều khiển thời gian thực khắt khe. Các nghiên cứu từ 2020 đến nay đã chuyển dịch mạnh mẽ từ các bộ điều khiển tuyến tính sang phi tuyến và dự báo.

Từ PID đến LQR (Linear Quadratic Regulator) Các thế hệ đầu (như CMU Ballbot) sử dụng các vòng lặp PID độc lập. Tuy nhiên, PID gặp khó khăn khi xử lý sự ràng buộc chéo (coupling) giữa các trục. Bộ điều khiển LQR đã trở thành tiêu chuẩn nhờ khả năng tối ưu hóa đa mục tiêu (cân bằng giữa sai số góc nghiêng và năng lượng tiêu thụ) thông qua việc giải phương trình Riccati [64].

Điều khiển Dự báo Mô hình (MPC - Model Predictive Control) Đây là xu hướng nghiên cứu nóng nhất hiện nay. Jespersen et al. (2020) [73] đã áp dụng thành công Path-Following MPC.

\begin{itemize}
    \item Ưu điểm: Khác với LQR chỉ phản ứng với sai số hiện tại, MPC sử dụng mô hình động lực học để "dự đoán" trạng thái của robot trong một khoảng thời gian tương lai (prediction horizon).
    \item Ứng dụng: MPC cho phép tích hợp trực tiếp các ràng buộc vật lý (như giới hạn điện áp động cơ, góc nghiêng tối đa) vào bài toán tối ưu. Điều này giúp robot thực hiện các quỹ đạo lượn phức tạp ở tốc độ cao mà không bị bão hòa động cơ dẫn đến ngã.
\end{itemize}

Điều khiển Bền vững và Thông minh (Robust \& Intelligent Control) Trong thực tế, các tham số như khối lượng robot thay đổi khi mang tải. Lee và Park (2020) [67] đã đề xuất các luật điều khiển bền vững (Robust Control) để bù trừ sai số mô hình. Mới đây nhất, Kayacan [72] đề xuất sử dụng mạng nơ-ron mờ (Fuzzy Neural Network) để học các đặc tính ma sát phi tuyến của sàn nhà, giúp robot vận hành mượt mà trên nhiều loại bề mặt khác nhau.

\subsubsection{Thực nghiệm và Ứng dụng}

Các nền tảng tiêu biểu:
\begin{itemize}
    \item Robot Rezero (ETH Zurich): Fankhauser [63] đã chế tạo robot Ballbot có khả năng di chuyển với tốc độ lên tới 2-3 m/s và thực hiện các động tác nghiêng người linh hoạt, mở ra tiềm năng ứng dụng cho các robot dịch vụ.
    \item Các thiết kế mới (2024-2025): Các công bố mới nhất của de Jesus [70] và Park [75] trên IEEE Access đã trình bày quy trình thiết kế Ballbot chi phí thấp tích hợp cảm biến IMU hiện đại và thuật toán Sensor Fusion để triệt tiêu sai số trôi (drift) của góc Yaw - vấn đề đau đầu nhất của Ballbot.
\end{itemize}

An toàn và Cơ chế Phục hồi Vì trạng thái tự nhiên là "đổ", an toàn là vấn đề sống còn. Skrabel [71] nhấn mạnh tầm quan trọng của cơ chế chân chống tự động (retractable legs). Các chân này phải bung ra trong mili-giây khi hệ thống phát hiện sự cố hoặc mất nguồn, ngăn chặn va chạm với con người.

Với ưu điểm vượt trội về khả năng di chuyển toàn hướng trong không gian hẹp và hình dáng thân thiện với con người, Ballbot đang dần chuyển từ phòng thí nghiệm sang các ứng dụng thực tế như robot lễ tân, robot hỗ trợ y tế, dù vẫn còn thách thức về chi phí và an toàn [74].
...
\bibitem{mir2022} I. Mir et al., “A survey of trajectory planning techniques for autonomous systems,” Electronics, vol. 11, no. 18, p. 2801, Sep. 2022, doi: 10.3390/electronics11182801.
...
\bibitem{zuo2024} X. Zuo et al., "Visual-Based Kinematics and Pose Estimation for Skid-Steering Robots," in IEEE Transactions on Automation Science and Engineering, vol. 21, no. 1, pp. 91-105, Jan. 2024, doi: 10.1109/TASE.2022.3214984.
...
% ----- BẮT ĐẦU: CẤU TRÚC DẪN ĐỘNG BÁNH CẦU -----
...
\bibitem{fankhauser2010} P. Fankhauser and C. Gwerder, "Modeling and Control of a Ballbot," Bachelor's Thesis, Swiss Federal Institute of Technology (ETH), Zurich, Switzerland, 2010.
...
\bibitem{lee2020} S. -M. Lee and B. S. Park, "Robust Control for Trajectory Tracking and Balancing of a Ballbot," in IEEE Access, vol. 8, pp. 159324-159330, 2020, doi: 10.1109/ACCESS.2020.3020091.
\bibitem{han2014} H. Y. Han, T. Y. Han and H. S. Jo, "Development of omnidirectional self-balancing robot," 2014 IEEE International Symposium on Robotics and Manufacturing Automation (ROMA), Kuala Lumpur, Malaysia, 2014, pp. 57-62, doi: 10.1109/ROMA.2014.7295862.
\bibitem{bonci2016} A. Bonci, "New dynamic model for a Ballbot system," 2016 12th IEEE/ASME International Conference on Mechatronic and Embedded Systems and Applications (MESA), Auckland, New Zealand, 2016, pp. 1-6, doi: 10.1109/MESA.2016.7587176.
\bibitem{jesus2024} P. H. de Jesus, C. L. N. Júnior, D. S. Dos Santos and S. R. B. Dos Santos, "Design and Experimental Validation of a Ballbot: A Self-balancing Omnidirectional Robot," 2024 IEEE International Systems Conference (SysCon), Montreal, QC, Canada, 2024, pp. 1-8, doi: 10.1109/SysCon61195.2024.10553418.
\bibitem{skrabel2013} C. Skrabel, "Mechanical Design of a Ballbot Platform," Bachelor's Thesis, Swiss Federal Institute of Technology (ETH), Zurich, Switzerland, 2013.
\bibitem{pham2024} A. Pham, B. H. Thai, P. V. Dang, and N. T. Vo, "Analysis of the parametric configuration impact on BallBot control performance," International Journal of Mechanical System Dynamics, vol. 4, no. 4, pp. 446-460, Nov. 2024, doi: 10.1002/msd2.12133.
\bibitem{jespersen2020} T. K. Jespersen et al., "Path-Following Model Predictive Control of Ballbots," 2020 IEEE International Conference on Robotics and Automation (ICRA), Paris, France, 2020, pp. 1498-1504, doi: 10.1109/ICRA40945.2020.9196634.
\bibitem{hollis2006} R. Hollis, "BALLBOTS." Scientific American, vol. 295, no. 4, 2006, pp. 72-77. JSTOR, http://www.jstor.org/stable/26069005.
...
% ----- KẾT THÚC: CẤU TRÚC DẪN ĐỘNG BÁNH CẦU -----
